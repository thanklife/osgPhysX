...
#else
    const int SWEPT_INTEGRATION_LINEAR = 1;
...
#else
    PxSceneQueryFilterFlags& queryFlags )
#endif
{
    ...
    return ((filterData1.word3 & VehicleManager::COLLISION_FLAG_DRIVABLE_SURFACE)==0) ?
           PxSceneQueryHitType::eNONE : PxSceneQueryHitType::eBLOCK;
}
...
{
    PxRigidActor* rigidActor = actor ? actor->isRigidActor() : NULL;
    if ( rigidActor )
    {
    ...
    {
        PxParticleBase* particleBase = actor->isParticleBase();
        if ( particleBase )
        {
            PxFilterData filter;
            filter.word0 = COLLISION_FLAG_OBSTACLE;
...
#else
void VehicleManager::update( double step, const std::string& s, std::vector<PxVehicleWheels*>& vehicles, unsigned int numWheels )
{
    PxScene* scene = Engine::instance()->getScene(s);
    if ( !scene || step<=0.0 ) return;
    updateQueryData( scene, numWheels );
    ...
    unsigned int size = vehicles.size();
    PxVehicleSuspensionRaycasts( _query, size, &(vehicles[0]), _numQueries, _queryResults );
    PxVehicleUpdates( step, scene->getGravity(), *_surfaceTirePairs, size, &(vehicles[0]) );
}
#endif
...
#else
        PxBatchQueryDesc queryDesc;
        queryDesc.userRaycastResultBuffer = _queryResults;
        queryDesc.userRaycastHitBuffer = _queryHitBuffer;
        queryDesc.raycastHitBufferSize = _numQueries;
#endif
        queryDesc.preFilterShader = wheelRaycastPreFilter;
        queryDesc.spuPreFilterShader = NULL;
        queryDesc.spuPreFilterShaderSize = 0;

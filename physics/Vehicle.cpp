...
#   include <vehicle/PxVehicleUtil.h>
    ...
    {
        enum PxVehicleDrive4WWheelOrderEnum
        {
            eFRONT_LEFT = physx::PxVehicleDrive4W::eFRONT_LEFT_WHEEL,
            eREAR_LEFT = physx::PxVehicleDrive4W::eREAR_LEFT_WHEEL,
            eFRONT_RIGHT = physx::PxVehicleDrive4W::eFRONT_RIGHT_WHEEL,
            eREAR_RIGHT = physx::PxVehicleDrive4W::eREAR_RIGHT_WHEEL,
        };
    };
#endif
...
#else
        PxTransform t = PxShapeExt::getGlobalPose(*(shapes[i]));
#endif
...
{
    // Create the actor
    physx::PxRigidDynamic* actor = SDK_OBJ->createRigidDynamic( PxTransform::createIdentity() );
    if ( !actor ) return false;
    
    actor->setMass( chassis.mass );
    actor->setMassSpaceInertiaTensor( chassis.moi );
    actor->setCMassLocalPose( PxTransform(chassis.massOffsetCenter, PxQuat::createIdentity()) );
    ...
    for ( int i=0; i<numWheels; ++i )
    {
        ...
        _wheelShapes[i]->setLocalPose( PxTransform::createIdentity() );
    ...
    _chassisShape->setLocalPose( PxTransform::createIdentity() );
...
#else
        PxVehicleDrive4WSmoothDigitalRawInputsAndSetAnalogInputs(
            g_smoothingData, _speedTable, _controlData, step, *drive4W );
#endif
    }
    if ( _reverseMode ) swapAccelerationAndBrake();
}
...
#else
    for ( int i=0; i<4; ++i ) drive4W->setWheelShapeMapping( i, i );
#endif
...
#else
    for ( int i=0; i<4; ++i ) drive4W->setSceneQueryFilterData( i, queryFilterData );
#endif
    return true;
}

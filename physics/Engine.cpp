...
#if USE_PHYSX_33
PxCudaContextManager* Engine::getOrCreateCudaContextManager( PxCudaContextManagerDesc* desc, PxProfileZoneManager* zoneMgr,
                                                             bool forceCreating )
{
    ...
    {
        if ( desc )
            _cudaManager = PxCreateCudaContextManager( _physicsSDK->getFoundation(), *desc, zoneMgr );
        else
        {
            physx::PxCudaContextManagerDesc defDesc;
            defDesc.interopMode = PxCudaInteropMode::OGL_INTEROP;
            _cudaManager = PxCreateCudaContextManager( _physicsSDK->getFoundation(), defDesc, zoneMgr );
        }
    }
    return _cudaManager;
}
